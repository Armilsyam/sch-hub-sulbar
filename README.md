# sch-hub-sulbar
 SCH-Hub Sulawesi Barat : Dashboard interaktif dengan Streamlit adalah cara yang sangat modern dan efektif untuk mempresentasikan gagasan kepada pemangku kepentingan (pemerintah, investor, atau akademisi).

 Berikut adalah panduan lengkap proyek Anda:

1. Struktur Repositori GitHub
Buatlah repositori baru di GitHub (misalnya dengan nama sch-hub-sulbar), lalu buat struktur file seperti berikut:
...
└── README.md              # Penjelasan proyek untuk GitHub
2. Isi File requirements.txt
Buat file bernama requirements.txt dan isi dengan baris berikut. Ini akan memberitahu server Streamlit library apa saja yang perlu diinstal.
...
numpy
plotly
3. Isi File app.py (Kode Utama Streamlit)
Ini adalah kode Python yang akan merender dashboard interaktif Anda, lengkap dengan analisis data dari abstrak dan proyeksi/prediksi masa depan. Salin kode ini ke dalam file app.py:
...
4. Cara Deploy ke Streamlit Community Cloud
Setelah Anda mengunggah ketiga file tersebut (app.py, requirements.txt, dan README.md) ke repositori GitHub Anda, ikuti langkah berikut:

Buka share.streamlit.io dan login menggunakan akun GitHub Anda.

Klik tombol "New app".

Pilih repositori yang baru saja Anda buat (misal: sch-hub-sulbar).

Pastikan Main file path terisi dengan app.py.

Klik "Deploy!".

Tunggu beberapa saat. Aplikasi Anda akan online dan Anda akan mendapatkan tautan (URL) yang bisa dibagikan langsung ke klien Anda.
